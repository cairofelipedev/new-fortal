...
class EventSpacesImport implements ToCollection, WithHeadingRow
{
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) {
            ...
            // Busca ou cria o tipo de evento
            $eventType = EventType::where('name', $row['type'])->first(); // Ajuste 'type' conforme o nome na planilha

            // Atualiza ou cria o espaço de evento
            EventSpace::updateOrCreate(
                [
                    'name' => $row['name'], // Condição para identificar o registro (por exemplo, usando o nome como chave única)
                ],
                [
                    'company_id' => $company ? $company->id : null, // Usa o id ou null se não encontrar
                    'city' => $row['city'],
                    'address' => $row['address'],
                    'respondent_full_name' => $row['respondent_full_name'],
                    'respondent_whatsapp' => $row['respondent_whatsapp'],
                    'type' => $eventType ? $eventType->id : null, // Usa o ID do tipo de evento ou null
                    // 'type' => $row['type'],
